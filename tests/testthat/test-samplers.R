...
test_that("sampler_rejection: points have correct dimension", {
  ...
  s <- sampler_rejection(is_in)
  res <- s(100L, t = c(0.5, 0.5), h = 0.3)
  expect_equal(nrow(res$points), 2L)
  expect_true(ncol(res$points) > 0L)
})
...
test_that("sampler_rejection: n_total equals N_quad", {
  ...
  is_in <- function(...) rep(TRUE, length(..1))
  s <- sampler_rejection(is_in)
  res <- s(200L, t = c(0.5, 0.5), h = 0.3)
  expect_equal(res$n_total, 200L)
})
...
test_that("sampler_rejection: all points lie in [-h, h]^d", {
  ...
  is_in <- function(...) rep(TRUE, length(..1))
  s <- sampler_rejection(is_in)
  res <- s(300L, t = c(0.5, 0.5), h = h)
  expect_true(all(abs(res$points) <= h + 1e-9))
})
...
test_that("sampler_rejection: points respect domain constraint", {
  ...
  s <- sampler_rejection(is_in)
  res <- s(300L, t = c(0, 0), h = 0.5)
  pts_global <- sweep(t(res$points), 2L, c(0, 0), "+")
...
test_that("sampler_rejection: error when V(h) is empty", {
  ...
  s <- sampler_rejection(is_in)
  expect_error(s(100L, t = c(0.5, 0.5), h = 0.3))
})
...
test_that("sampler_sector: returns exactly N_quad points of dimension 2", {
  set.seed(10L)
  s <- sampler_sector(2)
  res <- s(300L, t = c(0.5, 0.1), h = 0.2)
  expect_equal(nrow(res$points), 2L)
  expect_equal(ncol(res$points), 300L)
})
...
test_that("sampler_sector: points lie within D_k", {
  ...
  t_pt <- c(0.5, 0.1)
  s <- sampler_sector(k)
  res <- s(300L, t = t_pt, h = 0.2)
  pts <- sweep(t(res$points), 2L, t_pt, "+")
  x <- pts[, 1L]
  y <- pts[, 2L]
...
test_that("sampler_sector: n_total > N_quad near apex (small volume)", {
  ...
  N <- 200L
  s <- sampler_sector(2)
  res <- s(N, t = c(0.05, 0.01), h = 0.1)
  expect_gt(res$n_total, N)
})

test_that("sampler_sector: error when x range is empty", {
  s <- sampler_sector(2)
  expect_error(s(100L, t = c(-0.5, 0), h = 0.3))
})
...
test_that("sampler_owin: returns N points of dimension 2", {
  ...
  res <- s(100L, t = c(0.5, 0.5), h = 0.3)
  expect_equal(nrow(res$points), 2L)
  expect_equal(ncol(res$points), 100L)
})
...
test_that("sampler_owin: points lie within the owin", {
  ...
  h <- 0.4
  s <- sampler_owin(win)
  res <- s(150L, t = t_pt, h = h)
  pts_x <- res$points[1L, ] + t_pt[1L]
  pts_y <- res$points[2L, ] + t_pt[2L]
