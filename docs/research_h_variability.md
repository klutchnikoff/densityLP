# [BROUILLON / ARCHIVE] Variabilité de selection de h

> **Note importante :** Ce document est un brouillon de recherche. Il utilise un **noyau gaussien** à des fins de simplification mathématique (linéarisation), ce qui est **incohérent** avec le noyau "box" (uniforme) utilisé dans `densityLP`. L'approche décrite ici (variabilité de $\hat{h}$) n'est pas implémentée dans la version actuelle du package.

---

# Estimation de la variance de $\hat{f}_{\hat{h}}(x)$ — Guide d'implémentation (Brouillon)
